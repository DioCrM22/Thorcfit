...
const ForgotPassword = () => {
  ...
  const handleForgot = async () => {
    ...
    try {
      ...
      if (errorMessage === "Email não localizado no sistema") { 
        notify("❌ Email não cadastrado", "error");
        setEmail("");  // Limpa o campo
      } else if (errorMessage) {
        notify(`❌ ${errorMessage}`, "error");
      } else {
        notify("✅ Email localizado", "success");
        setShowPopup(true);
      }
    } catch (err) {
    ...
    try {
      ...
      if (errorMessage) {
        if (errorMessage.includes("igual")) {
          ...
          notify("❌ Não pode ser a senha anterior", "error");
        } else {
          notify(`❌ ${errorMessage}`, "error");
        }
      } else {
